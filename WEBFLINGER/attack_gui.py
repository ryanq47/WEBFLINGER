...
bedb.loadCOLUMN("slap_db","slap_ip_table", targetIP,'open_ports')
gui_ports = bedb.loadCOLUMN.key_data
	
	## -- loading services from DB -- ##
bedb.loadCOLUMN("slap_db","slap_ip_table", targetIP,'services')	
gui_services = bedb.loadCOLUMN.key_data

	## -- loading scanned range from DB -- ##
bedb.loadCOLUMN("slap_db","slap_ip_table", targetIP,'scanned_range')	
gui_scanned_ranges = bedb.loadCOLUMN.key_data

	## -- loading nmap scan from db -- ##
bedb.loadCOLUMN("slap_db","slap_ip_table", targetIP,'nmap_scan')
nmap_scan = bedb.loadCOLUMN.key_data

	## -- loading ping data from tmp -- ##
...
def make_slap_main() -> Panel:
    ...
    slap_main.add_column(style=rand_color, justify="left")
...
class Header:
    ...
    def __rich__(self) -> Panel:
        ...
        grid.add_row(
            "[b]" + targetIP + " Info [/b]  - [b blink] Press ENTER to start the attack!",
            datetime.now().ctime().replace(":", "[blink]:[/]"),
        )
        return Panel(grid, style="white on " + rand_color)
